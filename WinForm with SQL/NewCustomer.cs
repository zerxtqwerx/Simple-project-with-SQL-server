...
namespace WinForm_with_SQL
{
    public partial class NewCustomer : Form
    {
        ...
        private bool IsOrderDataValid()
        {
            ...
            {
                MessageBox.Show("Пожалуйста, создайте аккаунт перед размещением заказа.");
                return false;
            }
            ...
            if (IsCustomerNameValid())
            {
                ...
                using (SqlConnection connection = new SqlConnection(Properties.Settings.Default.connString))
                {
                    ...
                    using (SqlCommand sqlCommand = new SqlCommand("Sales.uspNewCustomer", connection))
                    {
                        ...
                        {
                            MessageBox.Show("Идентификатор клиента не был возвращен. Не удалось создать учетную запись.");
                        }
                        finally
                        {
                            connection.Close();
                ...
                using (SqlConnection connection = new SqlConnection(Properties.Settings.Default.connString))
                {
                    ...
                    using (SqlCommand sqlCommand = new SqlCommand("Sales.uspPlaceNewOrder", connection))
                    {
                        ...
                        try
                        {
                            ...
                            MessageBox.Show("Заказ номер " + this.orderID + " был размещен.");
                        }
                        catch
                        {
